	...
	const int max_gaussians_per_pixel)
{
  ...
  std::function<char*(size_t)> binningFunc = resizeFunctional(binningBuffer);
  std::function<char*(size_t)> imgFunc = resizeFunctional(imgBuffer);
  torch::Tensor winner_count = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
  ...
  if (P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forwardCountContribMax(
	    geomFunc,
		binningFunc,
		imgFunc,
	    P, degree, M,
...
std::tuple<torch::Tensor, torch::Tensor, int, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
CountGaussiansContribMaxResidualCUDA(
	const torch::Tensor& background,
	const torch::Tensor& means3D,
    const torch::Tensor& colors,
    const torch::Tensor& opacity,
	...
	const int max_gaussians_per_pixel)
{
  ...
  std::function<char*(size_t)> imgFunc = resizeFunctional(imgBuffer);
  torch::Tensor winner_count = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
  torch::Tensor winner_residual_score = torch::full({P}, 0.0, float_opts);
  ...
  if (P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forwardCountContribMaxResidual(
	    geomFunc,
		binningFunc,
		imgFunc,
	    P, degree, M,
		...
		tan_fovy,
		prefiltered,
		residual_map.contiguous().data<float>(),
		out_color.contiguous().data<float>(),
		winner_count.contiguous().data<int>(),
		winner_residual_score.contiguous().data<float>(),
		radii.contiguous().data<int>(),
		debug,
		max_gaussians_per_pixel);
  }
  return std::make_tuple(winner_count, winner_residual_score, rendered, out_color, radii, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const int max_gaussians_per_pixel)
{
  ...
  if (P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forwardCountAreaMax(
	    geomFunc,
		binningFunc,
		imgFunc,
	    P, degree, M,
...
std::tuple<torch::Tensor, torch::Tensor, int, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
CountGaussiansAreaMaxResidualCUDA(
	const torch::Tensor& background,
	const torch::Tensor& means3D,
    const torch::Tensor& colors,
    const torch::Tensor& opacity,
	...
	const int max_gaussians_per_pixel)
{
  ...
  if (P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forwardCountAreaMaxResidual(
	    geomFunc,
		binningFunc,
		imgFunc,
	    P, degree, M,
	...
	const int max_gaussians_per_pixel)
{
  ...
  // Initialize gaussians_count, important_score, and weighted_important_score
  torch::Tensor gaussians_count = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
  torch::Tensor important_score = torch::full({P}, 0.0, float_opts);
  torch::Tensor weighted_important_score = torch::full({P}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  // Use forwardCountWeighted
	  rendered = CudaRasterizer::Rasterizer::forwardCountWeighted(
	    geomFunc,
		binningFunc,
		imgFunc,
	    P, degree, M,
...
CountGaussiansWeightedResidualCUDA(
	const torch::Tensor& background,
	const torch::Tensor& means3D,
    const torch::Tensor& colors,
    const torch::Tensor& opacity,
	...
	const int max_gaussians_per_pixel)
{
  ...
  const int P = means3D.size(0); const int H = image_height; const int W = image_width; 
  auto float_opts = means3D.options().dtype(torch::kFloat32); 
  torch::Tensor out_color = torch::full({NUM_CHANNELS, H, W}, 0.0, float_opts); torch::Tensor radii = torch::full({P}, 0, means3D.options().dtype(torch::kInt32)); torch::Device device(torch::kCUDA); torch::TensorOptions options(torch::kByte); torch::Tensor geomBuffer = torch::empty({0}, options.device(device)); torch::Tensor binningBuffer = torch::empty({0}, options.device(device)); torch::Tensor imgBuffer = torch::empty({0}, options.device(device)); std::function<char*(size_t)> geomFunc = resizeFunctional(geomBuffer); std::function<char*(size_t)> binningFunc = resizeFunctional(binningBuffer); std::function<char*(size_t)> imgFunc = resizeFunctional(imgBuffer); 
  torch::Tensor gaussians_count = torch::full({P}, 0, means3D.options().dtype(torch::kInt32)); torch::Tensor important_score = torch::full({P}, 0.0, float_opts); torch::Tensor weighted_important_score = torch::full({P}, 0.0, float_opts); torch::Tensor weighted_residual_score = torch::full({P}, 0.0, float_opts); 
  int rendered = 0;  if (P != 0)
  {
	  int M = 0;	  if (sh.size(0) != 0)
	  {
		M = sh.size(1);      }
	  rendered = CudaRasterizer::Rasterizer::forwardCountWeightedResidual(
	    geomFunc,
		binningFunc,
		imgFunc,
	    P, degree, M,
...
std::tuple<torch::Tensor, int, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
DepthGaussiansWinnerCUDA(
	const torch::Tensor& background,
	const torch::Tensor& means3D,
    const torch::Tensor& colors,
    const torch::Tensor& opacity,
	...
	const int max_gaussians_per_pixel)
{
  ...
  auto float_opts = means3D.options().dtype(torch::kFloat32);
  torch::Tensor out_color = torch::full({NUM_CHANNELS, H, W}, 0.0, float_opts);
  torch::Tensor depth_map = torch::full({1, H, W}, 0.0, float_opts);
  torch::Tensor radii = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
  torch::Device device(torch::kCUDA);
  torch::TensorOptions options(torch::kByte);
  torch::Tensor geomBuffer = torch::empty({0}, options.device(device));
  ...
  if (P != 0)
  {
    ...
    rendered = CudaRasterizer::Rasterizer::forwardDepthWinner(
      ...
      tan_fovx, tan_fovy, prefiltered, out_color.contiguous().data<float>(),
      depth_map.contiguous().data<float>(), radii.contiguous().data<int>(), debug, max_gaussians_per_pixel);
  }
  return std::make_tuple(depth_map, rendered, out_color, radii, geomBuffer, binningBuffer, imgBuffer, torch::Tensor());
}

std::tuple<torch::Tensor, int, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
DepthGaussiansWeightedCUDA(
	const torch::Tensor& background,
	const torch::Tensor& means3D,
    const torch::Tensor& colors,
    const torch::Tensor& opacity,
	...
	const int max_gaussians_per_pixel)
{
  ...
  if (P != 0)
  {
    int M = 0;
    if (sh.size(0) != 0) { M = sh.size(1); }
    rendered = CudaRasterizer::Rasterizer::forwardDepthWeighted(
      geomFunc, binningFunc, imgFunc, P, degree, M,
      background.contiguous().data<float>(), W, H,
      means3D.contiguous().data<float>(), sh.contiguous().data_ptr<float>(),
      colors.contiguous().data<float>(), opacity.contiguous().data<float>(),
