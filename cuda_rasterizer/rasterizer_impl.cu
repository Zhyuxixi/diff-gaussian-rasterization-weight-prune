...
int CudaRasterizer::Rasterizer::forwardCountContribMax(
	std::function<char* (size_t)> geometryBuffer,
	std::function<char* (size_t)> binningBuffer,
	std::function<char* (size_t)> imageBuffer,
	const int P, int D, int M,
	...
	int max_gaussians_per_pixel)
{
	...
	const float* feature_ptr = colors_precomp != nullptr ? colors_precomp : geomState.rgb;
	CHECK_CUDA(FORWARD::count_gaussian_contrib_max(
		tile_grid, block,
		imgState.ranges,
		binningState.point_list,
		width, height,
...
int CudaRasterizer::Rasterizer::forwardCountAreaMax(
	std::function<char* (size_t)> geometryBuffer,
	std::function<char* (size_t)> binningBuffer,
	std::function<char* (size_t)> imageBuffer,
	const int P, int D, int M,
	...
	int max_gaussians_per_pixel)
{
	...
	const float* feature_ptr = colors_precomp != nullptr ? colors_precomp : geomState.rgb;
	CHECK_CUDA(FORWARD::count_gaussian_area_max(
		tile_grid, block,
		imgState.ranges,
		binningState.point_list,
		width, height,
...
// Forward with weighted residual score (alpha * T * residual)
int CudaRasterizer::Rasterizer::forwardCountWeightedResidual(
	std::function<char* (size_t)> geometryBuffer,
	std::function<char* (size_t)> binningBuffer,
	std::function<char* (size_t)> imageBuffer,
	const int P, int D, int M,
	...
	int max_gaussians_per_pixel)
{
	...
	dim3 tile_grid((width + BLOCK_X - 1) / BLOCK_X, (height + BLOCK_Y - 1) / BLOCK_Y, 1); dim3 block(BLOCK_X, BLOCK_Y, 1); // Dynamically resize image-based auxiliary buffers during training
	size_t img_chunk_size = required<ImageState>(width * height); char* img_chunkptr = imageBuffer(img_chunk_size); ImageState imgState = ImageState::fromChunk(img_chunkptr, width * height); if (NUM_CHANNELS != 3 && colors_precomp == nullptr)
	{
		throw std::runtime_error("For non-RGB, provide precomputed Gaussian colors!"); }
	...
	// Retrieve total number of Gaussian instances to launch and resize aux buffers
	int num_rendered; CHECK_CUDA(cudaMemcpy(
		&num_rendered,
		geomState.point_offsets + P - 1,
		sizeof(int),
		cudaMemcpyDeviceToHost), debug)

	size_t binning_chunk_size = required<BinningState>(num_rendered); char* binning_chunkptr = binningBuffer(binning_chunk_size); BinningState binningState = BinningState::fromChunk(binning_chunkptr, num_rendered); 
	// For each instance to be rendered, produce adequate [ tile | depth ] key
	// and corresponding dublicated Gaussian indices to be sorted
	duplicateWithKeys << <(P + 255) / 256, 256 >> > (
		P,
	...
	// Let each tile blend its range of Gaussians independently in parallel
	const float* feature_ptr = colors_precomp != nullptr ? colors_precomp : geomState.rgb; CHECK_CUDA(FORWARD::count_gaussian_weighted_residual(
		tile_grid, block,
		imgState.ranges,
		binningState.point_list,
		width, height,
...
int CudaRasterizer::Rasterizer::forwardCountAreaMaxResidual(
	std::function<char* (size_t)> geometryBuffer,
	std::function<char* (size_t)> binningBuffer,
	std::function<char* (size_t)> imageBuffer,
	const int P, int D, int M,
	...
	int max_gaussians_per_pixel)
{
	...
	const float* feature_ptr = colors_precomp != nullptr ? colors_precomp : geomState.rgb;
	CHECK_CUDA(FORWARD::count_gaussian_area_max_residual(
		tile_grid, block,
		imgState.ranges,
		binningState.point_list,
		width, height,
...
int CudaRasterizer::Rasterizer::forwardCountContribMaxResidual(
	std::function<char* (size_t)> geometryBuffer,
	std::function<char* (size_t)> binningBuffer,
	std::function<char* (size_t)> imageBuffer,
	const int P, int D, int M,
	...
	int max_gaussians_per_pixel)
{
	...
	const float* feature_ptr = colors_precomp != nullptr ? colors_precomp : geomState.rgb;
	CHECK_CUDA(FORWARD::count_gaussian_contrib_max_residual(
		tile_grid, block,
		imgState.ranges,
		binningState.point_list,
		width, height,
...
// Forward with weighted importance score (alpha * T)
int CudaRasterizer::Rasterizer::forwardCountWeighted(
	std::function<char* (size_t)> geometryBuffer,
	std::function<char* (size_t)> binningBuffer,
	std::function<char* (size_t)> imageBuffer,
	const int P, int D, int M,
	...
	int max_gaussians_per_pixel)
{
	...
	// Let each tile blend its range of Gaussians independently in parallel
	const float* feature_ptr = colors_precomp != nullptr ? colors_precomp : geomState.rgb;
	CHECK_CUDA(FORWARD::count_gaussian_weighted(
		tile_grid, block,
		imgState.ranges,
		binningState.point_list,
		width, height,
