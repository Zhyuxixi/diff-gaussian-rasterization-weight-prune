...
class GaussianRasterizer(nn.Module):
    ...
    def forward_count_weighted(self, means3D, means2D, opacities, shs = None, colors_precomp = None, scales = None, rotations = None, cov3D_precomp = None):
        """
        Forward pass with weighted importance score (alpha * T).
        Returns: gaussians_count, important_score, weighted_important_score, color, radii
        """
        raster_settings = self.raster_settings
        ...
        # Invoke C++/CUDA rasterization routine with weighted counting
        args = (
            raster_settings.bg,
            means3D,
            colors_precomp,
        ...
        if raster_settings.debug:
            ...
            try:
                gaussians_count, important_score, weighted_important_score, num_rendered, color, radii, geomBuffer, binningBuffer, imgBuffer = _C.count_gaussians_weighted(*args)
            except Exception as ex:
                torch.save(cpu_args, "snapshot_fw_weighted.dump")
                print("\nAn error occured in forward_count_weighted. Please forward snapshot_fw_weighted.dump for debugging.")
                raise ex
        else:
            gaussians_count, important_score, weighted_important_score, num_rendered, color, radii, geomBuffer, binningBuffer, imgBuffer = _C.count_gaussians_weighted(*args)
    ...
    def forward_count_contrib_max(self, means3D, means2D, opacities, shs=None, colors_precomp=None, scales=None, rotations=None, cov3D_precomp=None):
        """
        Forward pass with contrib-max winner counting.
        ...
        if raster_settings.debug:
            cpu_args = cpu_deep_copy_tuple(args)
            try:
                winner_count, num_rendered, color, radii, geomBuffer, binningBuffer, imgBuffer = _C.count_gaussians_contrib_max(*args)
            except Exception as ex:
                torch.save(cpu_args, "snapshot_fw_contrib_max.dump")
                print("\nAn error occured in forward_count_contrib_max. Please forward snapshot_fw_contrib_max.dump for debugging.")
                raise ex
        else:
            winner_count, num_rendered, color, radii, geomBuffer, binningBuffer, imgBuffer = _C.count_gaussians_contrib_max(*args)
    ...
    def forward_count_area_max_residual(
        self,
        means3D,
        means2D,
        opacities,
        ...
        """
        Forward pass with area-max winner counting plus winner residual accumulation.
        ...
        if residual_map is None:
            raise Exception('residual_map is required for forward_count_area_max_residual')
        ...
        if raster_settings.debug:
            cpu_args = cpu_deep_copy_tuple(args)
            try:
                winner_count, winner_residual_score, num_rendered, color, radii, geomBuffer, binningBuffer, imgBuffer = _C.count_gaussians_area_max_residual(*args)
            except Exception as ex:
                torch.save(cpu_args, "snapshot_fw_area_max_residual.dump")
                print("\nAn error occured in forward_count_area_max_residual. Please forward snapshot_fw_area_max_residual.dump for debugging.")
                raise ex
        else:
            winner_count, winner_residual_score, num_rendered, color, radii, geomBuffer, binningBuffer, imgBuffer = _C.count_gaussians_area_max_residual(*args)
